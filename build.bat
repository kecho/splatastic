@echo off
set build_type="%~1"
if %build_type% == "install-dev-debug" (
    set MESONPY_EDITABLE_VERBOSE=1
    py -m pip install -Csetup-args=-Dbuildtype=debug -Cbuild-dir=build-dbg --no-build-isolation --editable .
) else if %build_type% == "install-dev" (
    set MESONPY_EDITABLE_VERBOSE=1
