...
namespace InternalFileSystem
{
    ...
    OpaqueFileHandle openFile(const char* filename, RequestType request)
    {
        ...
        while (retry)
        {
            ...
            if (h == INVALID_HANDLE_VALUE)
            {
                ...
                {
                    Sleep(2.0f); //sleep for 2ms, if this is the 10th attempt, just fail.
                    continue;
                }
                return nullptr;
            }
    ...
    size_t fileSize(OpaqueFileHandle handle)
    {
        auto* wf = (WindowsFile*)handle;
        if (wf == nullptr)
            return {};

        SPT_ASSERT(wf->h != INVALID_HANDLE_VALUE);
        return wf->fileSize;
    }

    bool readBytes(OpaqueFileHandle h, char*& outputBuffer, int& bytesRead, bool& isEof)
    {
        SPT_ASSERT(h != nullptr);
        isEof = false;
        if (h == nullptr)
            return false;
    ...
    void getAbsolutePath(const std::string& path, std::string& outDir)
    {
        ...
        SPT_ASSERT_MSG(charsWritten != DestBufferSize, "getAbsolutePath failed, not enough characters (path too long)");
        if (charsWritten == 0)
        {
            outDir = path;
        }
...
namespace InternalFileSystem
{
    ...
    size_t fileSize(OpaqueFileHandle handle)
    {
        auto* pf = (PosixFile*)handle;
        if (pf == nullptr)
            return {};

        SPT_ASSERT(pf->h != 01);
        return (size_t)wf->fileSize;
    }
