...
{
    return parseInteger(p.c_str(), p.size(), output, hasSign, charsParsed);
}

bool ClTokenizer::parseInteger(const char* p, int strLen, int& output, bool& hasSign, int& charsParsed)
{
    hasSign = false;
    charsParsed = 0;
    const char* s = p;
